...
## gitignore 文件使用 ： 忽略工作区中的某类文件或文件夹，使之不会上传到仓库中
### 1， 创建 .gitignore文件
	每个文件均可以创建.gitignore，应用时，根据文件位置逐级调用，从而影响文件的版本控制状态。
	具体来说，子文件中的.gitignore会覆盖父文件夹下.gitignore对同一文件的控制，并且子文件的gitignore仅对子文件/子目录生效。
 	git check-ignore -v <filename> ：检查为什么某个文件被忽略了
	git status --ignored	： 显示所有被忽略的文件
### 2， 编辑文件
	1， 此文件作用是标记不被纳入版本控制的文件
	2， 文件的匹配是基于字符串的，而非正则表达式
	3， '#' 用来注释，添加说明
	4， '/' 作为开头，表示从根目录开始匹配
	5， '/' 作为结尾，表示匹配目录
	6， '!' 取消忽略，即取消上述某行的忽略
	7， 文件中每行表示一个需要忽略的模型，文件，文件夹，通配符
### 3， 示例
	/test 	忽略从根目录开始匹配到的所有test
	test  	忽略根目录下所有含有字符串test的文件和文件夹
	test.py	忽略根目录下所有test.py的python文件
	test.*	忽略根目录下所有test文件，不论类型。
	path/test.* 忽略path目录下所有test文件，不论类型
	！test.py 取消前面所有对test.py文件的忽略
### 4, 常见用法
	1，忽略日志文件：.log
	2, 忽略三方库: node_molules/
	3, 忽略可执行程序：*.exe
### 5，注意事项
	未被跟踪的文件可以通过.gitignore忽略跟踪
 	已经被跟踪的文件需要通过下列命令取消跟踪，之后放入.gitignore
  		: git rm -r --cached 文件
